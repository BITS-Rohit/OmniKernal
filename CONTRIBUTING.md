...
Welcome to OmniKernel! We are excited that you want to contribute. This guide will help you set up your development environment and understand the workflow.

## Project Vision

OmniKernel aims to provide a reliable, stealth-aware automation SDK. Our focus is on long-term maintainability and extensible architecture.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/OmniKernel.git
   cd OmniKernel
   ```
...
3. **Lint**: Run mandatory checks via pre-commit (`uv run pre-commit run --all-files` or `pip install pre-commit && pre-commit run --all-files`).
