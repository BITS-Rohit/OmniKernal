...
- `MessageType` StrEnum added to `src/omnikernal_core/global_metadata/msg_type.py`.
...
- **Plugin Architecture**: Decoupled the monorepo into three independent packages (`omnikernal`, `omnikernal-browser`, `omnikernal-whatsapp`) each independently versioned and publishable to PyPI.
...
- **Concurrent Logging**: Hardened `omnikernal_logger` with a graceful conditional import for `concurrent-log-handler`, falling back to a standard rotating handler when unavailable.
- **README**: Updated installation instructions for the decoupled package structure, updated all links to the `OmniKernel-Team` GitHub organization.
...
- **WhatsApp-Specific Code**: Extracted all WhatsApp platform code into `omnikernal-whatsapp`.
- **Browser-Specific Code**: Extracted all browser management code into `omnikernal-browser`.
