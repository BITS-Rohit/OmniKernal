...
Before submitting a Pull Request (PR), you **must** run the following linting and type-checking tools. PRs that fail
these checks will not be merged:

- `ruff`: Linting and formatting
- `pre-commit-hooks`: Formatting and whitespace fixes

You must run these checks before submitting. To run them, use `pre-commit`:
- Using `uv` (recommended): `uv run pre-commit run --all-files`
- Using `pip`: `pip install pre-commit && pre-commit run --all-files`
...
- For clarity and transparency, if your PR contains AI-generated or "AI-Vibe coded" logic, you **must** disclose this in
  the PR description.
...
- If your work involves **Underlying Web Selectors** (Playwright-based or JS Injection-based), these changes must be
  tested on high priority.
- These components are the foundation of OmniKernel and should not be submitted for review without thorough local
  testing.
- You must clearly state the reason for the change and what problem it intends to solve.

### 6. Branch & Isolation

- All **dev|docs work** should be directly pulled to dev branch , no **main** involvement.
- Add changes made should be added in  [CHANGELOG.md](https://github.com/BITS-Rohit/OmniKernel/blob/main/CHANGELOG.md)
...
Responsibilities of project maintainers include clarifying standards of acceptable behavior and taking appropriate and
