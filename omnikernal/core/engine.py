...
# from omnikernal.core.plugin_engine import PluginEngine
...
#         """Boot sequence."""
#         self.logger.info(
#             f"Booting OmniKernal for platform: {self.adapter.platform_name} [mode={self.mode}]"
#         )
...
#             self.repository, logger=self.logger, rules_cache=self._rules_cache
#         )
...
#         # 2. Get a per-request session/repo
...
#             dispatcher = EventDispatcher(repo, logger=self.logger, rules_cache=self._rules_cache)
#             watchdog = ApiWatchdog(repo)
#         else:
#             dispatcher = self.dispatcher  # type: ignore[assignment]
...
#         if result:
#             logged_cmd = (
#                 dispatch_result.command_name if dispatch_result else None
#             ) or clean_text.split(" ")[0][1:]
