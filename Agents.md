...
# OmniKernel SDK - Agent Guide

This file serves as a reference for AI agents interacting with `omnikernal`. It outlines the available capabilities, protocols, and utilities provided by the core SDK.

## Overview
`omnikernal` is the foundational layer of the OmniKernel SDK. It contains zero platform-specific logic (no WhatsApp or Browser code). Instead, it provides the standard interfaces (Protocols), encryption utilities, and shared metadata that all plugins must implement.
...
When modifying or extending `omnikernal`:
...
3. Import all core utilities via `from omnikernal_core import ...`.
